...
const WishCard: React.FC<WishCardProps> = ({ wish, onContribute }) => {
  ...
  return (
    <div className="bg-brand-light rounded-2xl overflow-hidden shadow-lg border border-gray-700 flex flex-col transition-transform hover:scale-[1.02] hover:shadow-base-blue/20">
      <img className="w-full h-56 object-cover" src={wish.imageUrl} alt={wish.name} />
      <div className="p-6 flex flex-col flex-grow">
        <h3 className="text-xl font-bold mb-2 text-white">{wish.name}</h3>
        <p className="text-xs text-gray-500 mb-4">
          by {`${wish.creatorAddress.slice(0, 6)}...${wish.creatorAddress.slice(-4)}`}
        </p>
